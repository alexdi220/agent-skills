...
compatibility: Requires .NET 6+ or .NET Framework 4.6.2+. NuGet packages from the DevExpress feed. DevExpress.Pdf.SkiaRenderer required for PDF export.
metadata:
  author: DevExpress
  version: "25.2"
...
| Package | Purpose |
|---------|---------|
| `DevExpress.Document.Processor` | Core presentation processing (create, load, edit, save, export) |
| `DevExpress.Pdf.SkiaRenderer` | Required for PDF export and printing (cross-platform rendering) |

### .NET (6/7/8+)

```bash
dotnet add package DevExpress.Document.Processor
dotnet add package DevExpress.Pdf.SkiaRenderer  # For PDF export and printing
```
...
> This feature requires DevExpress v26.1+. Reference to be added in a future update.
...
| PDF export produces no output | Missing `DevExpress.Pdf.SkiaRenderer` package | Add `dotnet add package DevExpress.Pdf.SkiaRenderer` |
...
2. **NuGet packages**: Use `DevExpress.Document.Processor` for core features. Add `DevExpress.Pdf.SkiaRenderer` for PDF export and printing. Do not guess other package names.
3. **Namespace imports**: Always include `using DevExpress.Docs.Presentation;` and other required directives (`DevExpress.Docs.Presentation.Export`, `DevExpress.Docs.Presentation.Printing`).

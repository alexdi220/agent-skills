...
In Visual Studio: go to **Tools > NuGet Package Manager > Package Manager Settings > Package Sources** and add `https://nuget.devexpress.com/api`.

In `nuget.config` (for .NET CLI projects):
```xml
<configuration>
  <packageSources>

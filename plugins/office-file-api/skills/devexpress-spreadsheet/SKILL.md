...
| `DevExpress.Pdf.SkiaRenderer` | Required for PDF export and printing on .NET (cross-platform) |
...
> On .NET Framework, PDF rendering uses Windows GDI — `DevExpress.Pdf.SkiaRenderer` is **not** required.
...
- Configure PDF export using Windows GDI (no SkiaRenderer)
...
When you need to:
- Export a workbook or worksheet to PDF
...
using (Workbook workbook = new Workbook())
{
    ...
    // Export to PDF (.NET only — requires DevExpress.Pdf.SkiaRenderer)
...
|---------|-------|----------|
| `FileNotFoundException` for SkiaSharp | Missing PDF renderer on .NET | Add `DevExpress.Pdf.SkiaRenderer` NuGet package |
...
2. **NuGet packages**: Use only `DevExpress.Document.Processor` (and `DevExpress.Pdf.SkiaRenderer` for PDF on .NET). Do not guess other package names.
...
7. **Framework detection**: Check .csproj for target framework. Use `Workbook` for both .NET and .NET Framework; SkiaRenderer only for .NET.

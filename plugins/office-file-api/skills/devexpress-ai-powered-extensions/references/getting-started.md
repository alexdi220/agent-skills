...
- DevExpress NuGet feed: `https://nuget.devexpress.com/api` — requires an active Universal Subscription or Office File API Subscription
- An AI provider account/endpoint (see provider options below)

## Step 1: Add the DevExpress NuGet Feed

Add the DevExpress feed to your NuGet configuration:

```bash
dotnet nuget add source https://nuget.devexpress.com/YOUR_DEVEXPRESS_FEED_KEY/api \
    --name DevExpress
```

Or add it through Visual Studio: **Tools → NuGet Package Manager → Package Sources**.
...
## Step 3: Register Your AI Provider (Console App)
...
## Step 4: Register in ASP.NET Core / Blazor
...
## Step 5: Create the Document Processing Service (Console)
...
## Step 6: Proofread a Word Document
...
## Step 7: Translate a Word Document
